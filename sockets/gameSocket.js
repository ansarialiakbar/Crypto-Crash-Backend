...
const setupGameSocket = (io) => {
    ...
    try {
      ...
      interval = setInterval(async () => {
        try {
          ...
          if (Date.now() - lastSaved >= 1000 && !isSaving) {
            isSaving = true;

            const currentRound = getCurrentRound();
            if (currentRound) {
              const freshRound = await GameRound.findById(currentRound._id);
              if (freshRound) {
                freshRound.multiplier = multiplier;
  ...
  // ✅ Start game loop immediately — assuming DB is already connected via server.js
  startRound();
  ...
  io.on('connection', (socket) => {
    ...
    if (currentRound) {
      ...
      socket.emit('multiplier_update', {
        multiplier: currentRound.multiplier?.toFixed(2) || '1.00',
      });
    } else {
